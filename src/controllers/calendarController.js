...
const getCalendarByShareCode = asyncHandler(async (req, res) => {
  try {
    ...
    if (!calendar) {
      return res.status(404).json({ success: false, message: '日历不存在或已过期' });
    }

    // 处理日期字段
    const createdAt = formatDate(calendar.created_at);
    const updatedAt = formatDate(calendar.updated_at);
    
    const deviceName = calendar.device ? calendar.device.device_name : 'Unknown Device';
    
    return res.json({
      name: calendar.title || '',
      color: calendar.color || 'ff2196f3',
      ownerId: calendar.device_id || '',
      createdAt,
      updatedAt
    });
  } catch (error) {
